...
// сделатть что бы данные дл€ входа и регестрации приходили из json запроса, который будет приходить на сервер, а пока дл€ тестов просто так
...
void AuthService::login(std::string user, std::string password)
{
	...
	auto createSession = std::make_shared<CreateSessionHandler>(userRepo); // генерирует JWT и сохран€ет в Ѕƒ
